...
class ResNet101Iris(models.ResNet):
...
class InceptionV3Iris(models.Inception3):
    ...
    def __init__(self, num_classes=1500):
        ...
        num_ftrs = self.AuxLogits.fc.in_features
        self.AuxLogits.fc = nn.Linear(num_ftrs, num_classes)
...
class EfficientNet(nn.Module):
    ...
    def extract_features_conv_stem(self, inputs):
        # Stem
        x = self._swish(self._bn0(self._conv_stem(inputs)))
        return x

    def extract_features_conv_stem_avg_pool(self, inputs):
        # Stem
        x = self._swish(self._bn0(self._conv_stem(inputs)))
        x = self._avg_pooling(x)
        return x

    def extract_features_blocks(self, inputs):
        # Stem
        x = self._swish(self._bn0(self._conv_stem(inputs)))
